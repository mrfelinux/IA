...
from tools_defs import TOOLS, _check_args, ToolDef
...
def _evaluate_single_tool(
    ...
    if len(tool_calls) > 1:
        called = [
            call.get("function", {}).get("name", "desconocida")
            for call in tool_calls
        ]
        details["tool_name"] = called[0] if called else None
        return (
            False,
            f"Demasiadas tool_calls ({len(tool_calls)}) para un test de una sola herramienta",
            f"PISTA: Se esperaba una sola llamada, recibidas: {called}.",
            details,
        )
...
def _evaluate_sequential(
    ...
    if len(tool_calls) > len(expected_seq):
        ...
        hint = (
            f"PISTA: Se esperaban exactamente {len(expected_seq)} tool_calls "
            f"pero se recibieron {len(tool_calls)}: {called}."
        )
        return False, f"Secuencia con llamadas extra ({len(tool_calls)}/{len(expected_seq)})", hint, details

    all_calls_info: list[dict[str, Any]] = []

    for i, (actual_call, expected) in enumerate(zip(tool_calls, expected_seq)):
